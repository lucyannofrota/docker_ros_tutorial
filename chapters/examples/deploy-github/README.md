...
2. Install necessary packages
    ...
    RUN apt-get update && \
        ...
        /bin/bash -c "git clone https://github.com/lucyannofrota/real-time-floor-demo.git ./src/floor_demo && \
                      ...
                      colcon build"
    ```
    Create the directory `/workspace` and set the current path to it. Then clone the Github repo to the folder `./src/floor_demo`, source ROS Humble, and finally build the package.
...
The service `floor_demo` uses a dockerfile with the instructions mentioned above to build an image called `humble_floor_demo`. Additionally, it uses the image `humble_floor_demo` to create a container named `floor_demo`; this container runs the command `". install/setup.bash && ros2 launch floor_demo floor_demo_launch.py"` when started.

**floor_demo service:**
```
floor_demo:
    image: humble_floor_demo
